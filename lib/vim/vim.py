...
def build_vim_S(num_classes:int):
    model = VisionMamba(
    ...
    model.default_cfg = _cfg()
    incompatibleKeys = model.load_state_dict(torch.load('/mnt/workspace/ZJ/HiFuseV2/lib/vim/vim_s_midclstok_80p5acc.pth'), strict=False)
    print(incompatibleKeys)
    model.head = nn.Linear(model.num_features, num_classes)
    return model

def build_vim_S_unpretrained(num_classes:int):
    model = VisionMamba(
        num_classes=num_classes, patch_size=16, embed_dim=384, depth=24, rms_norm=True, residual_in_fp32=True, fused_add_norm=True,
        final_pool_type='mean', if_abs_pos_embed=True, if_rope=False, if_rope_residual=False, bimamba_type="v2",
        if_cls_token=True, if_divide_out=True, use_middle_cls_token=True)
    model.default_cfg = _cfg()
    return model
...
def build_vim_T_unpretrained_256(num_classes:int):
    model = VisionMamba(
        img_size=256, num_classes=num_classes, patch_size=16, embed_dim=192, depth=24, rms_norm=True, residual_in_fp32=True, fused_add_norm=True, 
        final_pool_type='mean', if_abs_pos_embed=True, if_rope=False, if_rope_residual=False, bimamba_type="v2", if_cls_token=True, if_divide_out=True, use_middle_cls_token=True)
    model.default_cfg = _cfg()
    return model


def build_vim_S_unpretrained_256(num_classes:int):
    model = VisionMamba(
        img_size=256, num_classes=num_classes, patch_size=16, embed_dim=384, depth=24, rms_norm=True, residual_in_fp32=True, fused_add_norm=True,
        final_pool_type='mean', if_abs_pos_embed=True, if_rope=False, if_rope_residual=False, bimamba_type="v2",
        if_cls_token=True, if_divide_out=True, use_middle_cls_token=True)
    model.default_cfg = _cfg()
    return model
...
def build_vim_B_unpretrained_256(num_classes:int):
    model = VisionMamba(
        img_size=256, num_classes=num_classes, patch_size=16, embed_dim=768, d_state=16, depth=24, rms_norm=True, residual_in_fp32=True, fused_add_norm=True, 
        final_pool_type='mean', if_abs_pos_embed=True, if_rope=False, if_rope_residual=False, bimamba_type="v2", 
...
if __name__ == '__main__':
    model = build_vim_S_unpretrained(num_classes=3)